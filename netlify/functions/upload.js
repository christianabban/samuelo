...
async function saveChunk(fields, file) {
  ...
  const chunksStore = getStore({ name: 'upload-chunks', consistency: 'strong' });
...
async function completeUpload(fields) {
  ...
  const mediaStore = getStore({ name: 'media', consistency: 'strong' });
...
async function addManifestItem(albumId, item) {
  const manifestStore = getStore({ name: 'manifests', consistency: 'strong' });
